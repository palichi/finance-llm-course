...
이 실습은 **종목별로 쪼개진 여러 CSV가 아니라, `data/stock_prices.csv` 단 하나의
파일**을 씁니다. 이 파일 안에 **200개 종목의 데이터가 전부 섞여서 들어있습니다**
(이른바 long format — 종목코드라는 컬럼으로 행을 구분하는 방식).

```bash
head -3 data/stock_prices.csv
```

```
basDt,srtnCd,isinCd,itmsNm,mrktCtg,clpr,vs,fltRt,mkp,hipr,lopr,trqu,trPrc,lstgStCnt,mrktTotAmt
20240102,000080,KR7000080002,하이트진로,KOSPI,22300,-200,-.89,22500,22550,22300,203536,4556472800,70133611,1563979525300
20240103,000080,KR7000080002,하이트진로,KOSPI,22500,200,.9,22300,22550,22150,181964,4072012250,70133611,1578006247500
```

### 컬럼 의미 (금융위원회 공공데이터 형식)

| 컬럼명 | 의미 | 이번 실습에서의 역할 |
|---|---|---|
| `basDt` | 기준일자 (YYYYMMDD) | 시간축, 종목 내 정렬 기준 |
| `srtnCd` | 단축코드 (종목코드, 6자리) | **종목 구분 키** — 모든 종목별 분리 작업의 기준 |
| `isinCd` | ISIN 코드 | 사용 안 함 (참고용) |
| `itmsNm` | 종목명 | 챗봇에서 종목명으로 질문할 때 매핑용 |
| `mrktCtg` | 시장구분 (KOSPI/KOSDAQ) | 선택적 필터/메타데이터 |
| `clpr` | **종가** | 지표 계산의 기본값 |
| `vs` | 전일대비 | 사용 안 함 (참고용, fltRt와 중복 정보) |
| `fltRt` | 등락률(%) | 사용 안 함 (참고용) |
| `mkp` | **시가** | state 구성 시 참고 가능 |
| `hipr` | **고가** | state 구성 시 참고 가능 |
| `lopr` | **저가** | state 구성 시 참고 가능 |
| `trqu` | **거래량** | 있으면 신호 품질 향상 (이번 데이터는 거래량 포함!) |
| `trPrc` | 거래금액 | 선택 — 거래량×가격과 유사 정보 |
| `lstgStCnt` | 상장주식수 | 사용 안 함 |
| `mrktTotAmt` | 시장총액 | 사용 안 함 (참고용) |

> 💡 우리가 그동안 "종목코드/일자/시가/종가/고가/저가"라고 단순화해서 불렀던
> 컬럼들이 실제로는 `srtnCd/basDt/mkp/clpr/hipr/lopr`라는 이름으로 들어있습니다.
> 게다가 거래량(`trqu`)까지 이미 포함되어 있어서, "거래량이 있으면 신호 품질이
> 좋아진다"고 했던 부분을 바로 활용할 수 있습니다.

### 데이터 형태 관련 흔한 오해 — "한 파일에 합쳐져 있으면 종목별 학습이 안 되는 거 아닌가?"

**아닙니다.** 이전에 "모델은 1개로, 여러 종목 데이터를 합쳐서 학습한다"고
설명했던 것과, "파일이 종목별로 나뉘어 있는지 합쳐져 있는지"는 **완전히 별개의
문제**입니다.

| | 파일이 종목별로 분리 (`005930.csv`, `000660.csv`...) | **이번 실습: 파일 1개에 종목코드 컬럼으로 구분** |
|---|---|---|
| 종목을 구분하는 방법 | 파일명 자체 | `srtnCd` 컬럼 값으로 `groupby` |
| 지표 계산 시 경계 처리 | 파일 단위라 자동으로 분리됨 | **`srtnCd`별로 명시적으로 `groupby`해서 계산해야 함** (안 하면 종목이 섞여서 SMA/RSI가 망가짐) |
| 모델 학습 결과 | 동일 | 동일 |

즉 지금 가진 `stock_prices.csv` 형태 그대로 진행하면 됩니다. 다만 코드 안에서
"종목 경계를 절대 넘지 않는다"는 원칙을 지킬 때, 그 경계 기준이 "파일이 바뀌는
지점"이 아니라 **"`srtnCd` 값이 바뀌는 지점"**이 된다는 점만 다릅니다 — 아래
STEP 2/3 프롬프트에 이 기준을 명시해뒀습니다.

### 실제 데이터를 봤을 때 추가로 고려해야 하는 것

```bash
python3 -c "
import pandas as pd
df = pd.read_csv('data/stock_prices.csv', encoding='utf-8-sig', dtype={'srtnCd': str})
print('종목 수:', df['srtnCd'].nunique())
print('기간:', df['basDt'].min(), '~', df['basDt'].max())
print('종목당 행 수 범위:', df.groupby('srtnCd').size().min(), '~', df.groupby('srtnCd').size().max())
"
```

```
종목 수: 200
기간: 20240102 ~ 20260618
종목당 행 수 범위: 138 ~ 598
```

이 출력에서 보이듯, **종목마다 데이터가 있는 기간이 다릅니다** (200개 중
일부는 138~598행으로 행 수가 제각각입니다). 이건 최근 신규상장된 종목이거나,
데이터 수집 시점이 종목마다 달랐기 때문일 수 있습니다. 이 점은 STEP 1
설계 합의에서 미리 짚어서, "lookback 20일도 채 없는 종목은 학습/추론에서
어떻게 처리할지"를 정해두고 시작합니다.

---

## 📂 전체 폴더 구성 (실습이 끝나면 이렇게 됩니다)

지금 폴더에는 **이 `README.md`와 `data/stock_prices.csv`만 있습니다.**
나머지 파일/폴더는 전부 이번 실습에서 Claude Code로 직접 만듭니다.

```
ppo_rag_trading/
├── README.md                       ← 지금 보고 있는 이 파일 (이미 있음)
├── data/                            ← 이미 있음
│   └── stock_prices.csv             (200종목 통합, srtnCd로 종목 구분)
│
...
│
├── models/                          (자동 생성)
│   └── policy_ppo.zip               (200종목으로 학습된 단일 정책망)
│
...
200개 종목이 한 파일(`stock_prices.csv`)에 통합된 일자별 시세 데이터를
사용해서, **"지금 이 지표 상태 + 내 포지션에서 매수/매도/유보 중 무엇을
해야 자산가치가 늘어나는가"를 학습하는 PPO 강화학습 에이전트**를 만듭니다.
이것이 이번 실습의 **본체(主)** 입니다.
...
```
data/stock_prices.csv (200종목 통합, srtnCd로 구분)
        │
        ▼
indicators (srtnCd별 groupby 계산) ──▶ PPO 강화학습 (★ 주) ──▶ 매수/매도/유보 결정
                                  │
                                  ▼ (결정이 내려진 뒤에만 호출됨)
...
| 학습 단위 | **모델 1개** — 200종목의 episode를 섞어서 단일 정책망 학습 (종목별 모델 아님) |
| 종목 구분 키 | `srtnCd` 컬럼 — 지표 계산·episode 구성에서 **반드시 이 기준으로 분리** |
| 알고리즘/라이브러리 | PPO (stable-baselines3), 정책망 MLP `[64,64]`로 시작 |

### 데이터 — 우리가 가진 것과 이미 충분한 것

| 항목 | 컬럼명 | 상태 |
|---|---|---|
| 종목코드 | `srtnCd` | **있음** |
| 일자 | `basDt` | **있음** |
| 시가/종가/고가/저가 | `mkp`/`clpr`/`hipr`/`lopr` | **있음** |
| 거래량 | `trqu` | **있음** (그동안 "있으면 좋다"고 했던 게 이미 포함됨) |
| 종목명 | `itmsNm` | **있음** (챗봇에서 종목명으로 질의할 때 활용) |
| 초기 자본금/거래 단위 | - | 데이터에 없음 → **시뮬레이션 설정값**으로 직접 정의 |
| 거래비용/수수료율 | - | 데이터에 없음 → **가정값** (기본 0.2~0.3%) |
...
```bash
ls data/
#  stock_prices.csv 만 있는지 확인
```

```bash
head -3 data/stock_prices.csv
#  basDt,srtnCd,isinCd,itmsNm,mrktCtg,clpr,vs,fltRt,mkp,hipr,lopr,trqu,... 형태인지 확인
```
...
```
나는 주식 시세 데이터를 갖고 있어. data/stock_prices.csv 파일 하나에
200개 종목이 전부 통합되어 들어있어 (long format). 실제 컬럼은:
basDt(기준일자, YYYYMMDD), srtnCd(종목코드), isinCd(ISIN코드),
itmsNm(종목명), mrktCtg(시장구분 KOSPI/KOSDAQ), clpr(종가), vs(전일대비),
fltRt(등락률), mkp(시가), hipr(고가), lopr(저가), trqu(거래량),
trPrc(거래금액), lstgStCnt(상장주식수), mrktTotAmt(시장총액)

이 중 이번 모델에는 basDt, srtnCd, itmsNm, mkp, hipr, lopr, clpr, trqu만
사용할 거야. 종목 구분은 srtnCd 컬럼 값으로 한다는 걸 명확히 해줘.
...
- 모델은 하나만 학습하고, 200종목의 데이터를 합쳐서 학습 데이터로 사용해.
  단, 지표 계산과 episode(포지션·손익 시뮬레이션) 구성은 반드시 srtnCd
  기준으로 종목별로 분리해서 계산하고, 종목 경계를 넘어 섞이지 않게 해줘.
  학습 단계에서만 여러 종목의 episode를 섞어서 모델에 입력해.
- state에는 종목코드 자체를 포함하지 않아 — 모델이 지표 패턴의 일반 규칙을
  배우게 하려는 목적이야.
- 종목마다 데이터가 시작하는 시점이 다를 수 있어(신규상장 등으로 행 수가
  138~598개로 제각각이야). lookback 20일조차 없는 종목/구간은 학습·추론
  양쪽에서 어떻게 처리할지 합리적인 방법을 제안해줘.
...
- indicators/technical.py : 기술지표 계산 (srtnCd별 분리 계산)
- env/trading_env.py : gymnasium TradingEnv (srtnCd별 episode 경계 분리)
- train/reward.py : reward 셰이핑 함수
- train/run_training.py : PPO 학습 — 종목 episode를 섞어서 단일 모델
...
2. data/stock_prices.csv를 로딩해서 종목코드(srtnCd)와 종목명(itmsNm)
   매핑 테이블을 만드는 방법, 컬럼명을 내부적으로 어떻게 표준화(예:
   mkp→open, clpr→close 등 영문 표준명으로 rename)할지
3. 각 모듈의 입출력 인터페이스 (함수 시그니처 수준)
4. train/run_training.py에서 "여러 종목 episode를 섞는" 구체적 방식
5. lookback 20일이 채 안 되는 종목/구간 처리 방법
6. 모델(.zip)이 아직 없을 때와 있을 때 이후 만들 앱이 어떻게 다르게 동작해야
   하는지 — 학습은 별도 CLI 스크립트로 분리하고 앱은 추론만 담당할 것
7. 데이터 분할 전략 (시간 기준 train/validation, 종목 내에서 룩어헤드 누설 방지)

을 markdown으로 정리해서 보여줘. 내가 확인하면 순서대로 구현 시작하자.
```

> 💡 이 단계에서 Claude Code가 제안하는 "컬럼명 표준화 방식"과 "여러 종목
> episode를 섞는 방식"을 잘 읽어보세요. `srtnCd`/`clpr`/`mkp` 같은 원본
> 컬럼명을 그대로 코드 전체에서 쓰면 가독성이 떨어지니, 이후 모듈에서는
> `date`/`close`/`open` 같은 표준명으로 변환해서 쓰는 게 일반적입니다.
> 종목별 모델을 따로 두는 게 아니라 **모델 1개**로 일반화된 정책을 학습하는
> 게 이 실습의 핵심 설계입니다.
...
요구사항:
1. 입력: data/stock_prices.csv를 로딩한 pandas DataFrame. 원본 컬럼은
   basDt, srtnCd, itmsNm, mkp, hipr, lopr, clpr, trqu 이고, 함수 내부에서
   STEP 1에서 합의한 표준명(date, ticker, name, open, high, low, close,
   volume)으로 변환해서 사용해줘. basDt는 YYYYMMDD 정수이므로 datetime으로
   변환도 같이 해줘.
2. 다음 컬럼을 추가해서 반환:
   - SMA5, SMA20, SMA60
   - EMA20 (span=20)
...
3. ticker(원본 srtnCd) 별로 그룹화해서 계산하고, 종목 경계를 넘어 지표가
   섞이지 않게 할 것 (단위테스트로 반드시 검증: 서로 다른 ticker 두 개를
   이어붙인 가상 데이터를 만들어서 경계 지점의 지표가 다른 종목 값에
   영향받지 않는지 확인)
4. 종목별로 데이터 시작 시점이 다를 수 있음(신규상장 등) — 각 종목 내에서
   날짜 정렬 후 그 종목의 첫 행부터 지표를 계산하면 되고, 종목 간 날짜를
   억지로 맞추지 않아도 된다는 점을 주석으로 명시
5. 초기 구간(이동평균 계산 안 되는 첫 N일) NaN 처리는 옵션(drop/backfill)으로
6. docstring + pytest 단위테스트 (RSI는 알려진 예시값으로 검증, 컬럼명
   표준화가 올바르게 되는지도 테스트)
...
핵심 — episode 경계:
- 환경은 반드시 "단일 종목(ticker)" 데이터로만 구성되어야 함 (여러 종목을
  한 episode 안에서 이어붙이지 말 것)
- 이 종목의 데이터가 lookback 20일보다 짧으면(신규상장 등) 그 종목은
  episode 생성 대상에서 제외하거나, 데이터가 쌓일 때까지 건너뛰는 방식을
  제안해줘
...
바뀔 때 포지션/손익이 올바르게 초기화되는지, 데이터가 짧은 종목이
올바르게 걸러지는지 검증.
...
핵심 — 200종목 episode를 섞어서 단일 모델 학습:
- data/stock_prices.csv를 로딩하고, srtnCd별로 시간 기준 train/validation
  분리 (예: 특정 날짜 이전 train, 이후 validation)
- lookback 20일조차 확보하지 못하는 종목은 STEP 1~3에서 합의한 방식대로
  제외하거나 건너뛰기
...
종목별 학습 결과(종목코드, 종목명, 평균 episode return, 검증 적중 경향)를
요약하는 표를 마지막에 출력 (전체 200종목 중 상위/하위 몇 개만 보여줘도 됨).

먼저 종목 2~3개로 빠르게 50000 스텝 smoke run을 돌려서 파이프라인이
에러 없이 동작하는지 확인하고, 그 다음 본 학습(200종목 전체)을 논의하자.
...
#
# ===== 검증 구간 요약 (상위 5종목) =====
# 종목코드   종목명     누적수익률   MDD   샤프비율
# ...
```

> 예상 소요 시간: smoke run CPU 기준 약 3~10분, 200종목 본 학습은
> 데이터 규모(약 117,000행) 기준 CPU 수십 분~수시간, GPU 환경이면 단축
...
1. 입력: 종목코드(srtnCd) 또는 종목명(itmsNm) — 종목명으로 입력하면
   data/stock_prices.csv에서 매핑해서 srtnCd로 변환
2. 해당 종목 최근 데이터(lookback 20일 이상) 로드
   (데이터가 20일보다 짧으면 명확한 에러 메시지로 안내)
...
중 임의 종목·날짜 5곳을 뽑아 "그날 종가 흐름 표 + PPO 판단 + 수치 근거"를
화면에 출력하는 간단한 점검 스크립트 check_prediction.py도 같이 만들어줘.

출력 형식 예시:
  ──────────────────────────────────────
  [점검 1/5] 000080 하이트진로 (2025-06-01 기준)
  최근 5일 종가 흐름: 22,300 → 22,500 → 22,200 → 22,200 → 22,150원
...
```bash
pytest explain/ -v
python check_prediction.py --ticker 000080
# 또는
python check_prediction.py --name 하이트진로
```
...
이번 데이터는 200종목이라 RAG corpus에도 종목 다양성이 충분합니다 — 같은
패턴이 여러 종목에서 반복됐는지를 비교할 수 있다는 점에서 corpus 품질에
유리합니다.
...
1. 뉴스/공시 수집 (외부 데이터, 신규 수집 필요):
   - 종목코드(srtnCd) + 종목명(itmsNm) + 날짜를 입력하면 그 날짜 전후
     (예: -1일~+1일) 뉴스 기사 제목/요약을 가져오는 함수. 검색 키워드는
     종목명을 사용해줘 (종목코드만으로는 뉴스 검색이 잘 안 되는 경우가
     많음). 사용 가능한 뉴스 API가 있는지 먼저 물어보고, 없으면 네이버
     금융 뉴스 페이지 등에서 수집하는 방법을 제안해줘 (크롤링 시 약관/
     rate limit 준수 여부도 같이 짚어줘)
...
2. 지표 사례 카드 생성:
   - indicators/technical.py 와 inference/predict.py 결과를 이용해서,
     ...
     "{종목명}({종목코드}) {일자}: RSI14={값}({구간}), 이격도20={값}({구간}),
...
3. 임베딩 + 벡터DB 저장:
   ...
   - corpus/chroma_db에 ChromaDB로 저장: document(사례카드 텍스트),
     embedding(벡터), metadata(종목코드, 종목명, 일자, RSI14, disparity20,
...
200종목 전체로 한 번에 만들면 뉴스/공시 API 호출이 많아지니, 먼저 종목
2~3개, 최근 1개월 정도의 작은 범위로 corpus를 만들어서 구조가 잘
동작하는지 확인하고, 그 다음 전체 종목으로 확장하자.
...
# [corpus 구축] 000080 하이트진로, 2025-06-01 ~ 2025-06-30 (1개월 테스트 범위)
# 뉴스 수집: ... / 공시 수집: ...
# 사례 카드 생성: 20건
...
> 업데이트로 채우면 됩니다. 200종목 전체로 corpus를 만들면 API 호출량이
> 많아지니, 처음엔 관심 종목 몇 개로 좁혀서 진행하는 걸 권합니다.
...
3. Metadata Filtering:
   - 같은 종목 우선 vs 다른 종목(200종목 중)의 비슷한 패턴도 포함 여부를
     옵션으로 — 종목 수가 많아서 "다른 종목 사례 포함"이 특히 유용할 수
     있다는 점을 감안해줘
...
단위테스트: mock corpus(가상 사례 카드 10건, 여러 종목 섞어서)로 4가지
기법이 각각 의도대로 동작하는지 — metadata filter가 조건에 안 맞는 사례를
실제로 걸러내는지, reranking 전후로 순위가 바뀌는지 검증.
...
과거 사례를 인용할 때는 반드시 그 사례의 종목명(종목코드)과 날짜를 함께
언급해서 출처를 명확히 한다. 검색된 사례가 없으면 '참고할 과거 사례를
찾지 못했습니다'라고 명시하고 지어내지 않는다. 확신도가 낮게 표시된 경우 그
사실을 분명히 언급한다."

입력:
- rule_based 결과: action, action_probs, RSI14/disparity20 값과 구간,
  golden/dead flag, shaping 플래그, 확신도 낮음 플래그
- rag_retriever 결과: 재정렬된 Top-K 과거 사례 카드(종목코드, 종목명, 날짜,
  당시 지표값, 당시 PPO판단, 사후 수익률, 관련 뉴스/공시 텍스트)

출력: 4~6문장 한국어 설명. 현재 판단 근거를 먼저 설명하고, 그 다음
"과거 비슷한 사례에서는 ~"로 RAG 근거를 붙이는 구조. 인용한 사례는
종목명(종목코드)과 날짜를 함께 표기 (예: "하이트진로(000080), 2025-06-05")
...
- LLM 응답에 등장하는 모든 수치가 실제 값과 일치하는지 검증, 불일치 시
  폴백 템플릿 사용
- LLM 응답에 등장하는 종목명/종목코드+날짜 인용이 실제로 검색결과에
  존재하는 사례인지 검증 (없는 사례를 지어내 인용하는 것 방지)
...
```
[종목 하이트진로(000080)] 예측 결과: 매수 (확률 62%)
  - 기준일: 2025-06-05 / 종가: 22,150원
  - RSI14=58.2(중립), 이격도20=102.1(중립), 골든크로스 4일전 발생
  - 참고 근거 (Advanced RAG 검색 결과, 2건):
      1. 하이트진로(000080), 2024-11-12 — 비슷한 골든크로스 패턴, 이후 5일 +2.1%
      2. 삼성전자(005930), 2025-02-20 — RSI 중립대 매수 후 10일 +3.4%
  - 종합 설명: 현재 RSI와 이격도는 중립 구간이며 4일 전 골든크로스가
    발생해 추세 추종 보너스 조건에 부합합니다. 과거 유사 사례
    (하이트진로, 2024-11-12) 등에서도 비슷한 패턴 이후 단기 상승으로
...
1. 사용자 입력에서 종목코드 또는 종목명을 추출 — data/stock_prices.csv의
   itmsNm(종목명)/srtnCd(종목코드) 매핑을 이용. 200종목 중 종목명 일부만
   입력해도(예: "하이트") 유사 매칭으로 후보를 찾고, 여러 개면 selectbox로
   선택하게. 종목을 특정할 수 없으면 "어느 종목에 대해 궁금하신가요?"라고
   되묻기 (직전 메시지에서 종목이 언급됐으면 이어서 사용)
...
UI 구성:
- 사이드바: 종목코드/종목명 검색 가능한 selectbox(200종목 목록), 모델
  로딩 상태 표시, "대화 초기화" 버튼
- 메인 채팅창: 행동(매수/매도/유보)을 색상 배지로 강조, action_probs는
  작은 막대로 함께 표시
- "참고한 과거 사례" expander: Top-K 사례를 종목명(종목코드)/날짜/요약/
  사후수익률과 함께 나열
...
에러 처리: 존재하지 않는 종목코드/종목명, lookback보다 짧은 데이터(신규
상장 종목), RAG 검색 결과 0건(이 경우 "참고할 과거 사례를 찾지
못했습니다"로 명시)
...
부합합니다. 과거 유사 사례(하이트진로, 2024-11-12)에서도 비슷한 패턴
이후 단기 상승 경향이 있었으나, 이는 참고 사례일 뿐 결과를
보장하지 않습니다.

▸ 참고한 과거 사례 (펼치기)
  1. 하이트진로(000080) · 2024-11-12 · 5일후 +2.1% · 10일후 +3.8%
  2. 삼성전자(005930) · 2025-02-20 · 5일후 +3.4% · 10일후 +5.1%
...
data/stock_prices.csv                       data/stock_prices.csv +
(200종목 통합, srtnCd로 구분)                  뉴스/공시 API
      │                                            │
      ▼                                            ▼
indicators/technical.py                     corpus/build_corpus.py
(srtnCd별 groupby 계산)                      (사례 카드 + 임베딩)
      │                                            │
      ▼                                            ▼
env/trading_env.py                          corpus/chroma_db/
(srtnCd별 episode 분리)                       (벡터DB + BM25 인덱스)
      │                                            │
      ▼                                            │
train/run_training.py                              │
(200종목 episode를 섞어서 학습)                      │
      │                                            │
      ▼                                            │
...
**Q. 데이터가 종목별로 쪼개진 파일이 아니라 하나의 CSV에 다 섞여 있는데,
"여러 종목을 합쳐서 학습"이라는 게 가능한가요?**
```
네, 오히려 이번처럼 한 파일에 200종목이 들어있는 형태(long format)가
"여러 종목을 합쳐서 학습"하기에 자연스러운 구조입니다. 파일이 종목별로
쪼개져 있든(005930.csv, 000660.csv...) 한 파일에 srtnCd 컬럼으로 구분돼
있든, 모델 학습 결과는 동일합니다. 차이는 "종목 경계를 어떻게 분리하느냐"
뿐입니다 — 이번엔 파일명이 아니라 srtnCd 컬럼 값을 기준으로 groupby해서
분리합니다. indicators/technical.py(STEP 2)와 env/trading_env.py(STEP 3)
에서 이 분리를 명시적으로 구현하도록 프롬프트에 반영했습니다.
```

**Q. 컬럼명이 basDt, srtnCd, clpr처럼 낯선데 그대로 코드에 써도 되나요?**
```
원본 컬럼명을 그대로 써도 동작은 하지만, 가독성을 위해 STEP 1에서
표준명(date, ticker, name, open, high, low, close, volume)으로 변환해서
이후 모듈에서는 표준명만 사용하도록 설계했습니다. 원본 컬럼은
indicators/technical.py 진입 시점에 한 번만 변환됩니다.
```

**Q. 종목마다 데이터 행 수가 138~598개로 다른데 문제가 되나요?**
```
신규상장 등으로 종목별 데이터 기간이 다른 건 정상입니다. 문제가 되는
경우는 lookback 20일조차 확보되지 않은 종목/구간뿐이며, 이런 경우는
STEP 1에서 합의한 방식대로 학습·추론 대상에서 제외하거나 데이터가
쌓일 때까지 건너뛰도록 처리합니다.
```
...
```
아닙니다. 이 실습은 모델 1개를 200종목의 episode를 섞어서 학습합니다.
단, 지표 계산(STEP 2)과 episode 구성(STEP 3)은 srtnCd 기준으로 반드시
분리해서 계산하고, 학습 큐에 넣을 때만(STEP 5) 섞습니다. 종목코드 자체는
...
**Q. GPU가 없는데 가능한가요?**
```
PPO 학습과 임베딩 계산 모두 CPU로 가능합니다 (단, 200종목·117,000여
행 규모라 시간이 더 걸립니다). GPU 환경에서 실행하면 훨씬 빠릅니다.
```
